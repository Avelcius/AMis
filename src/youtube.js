...
const findVideo = async (songTitle, songArtist, spotifyDurationMs) => {
    try {
        ...
        console.log(`Searching YouTube for: "${searchQuery}" with duration ${spotifyDurationMs}ms`);
        // Fetch top 5 results to have a small pool for comparison
        const videos = await YouTube.search(searchQuery, { limit: 5, type: 'video' });
